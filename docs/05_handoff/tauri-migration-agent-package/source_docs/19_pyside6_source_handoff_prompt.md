...
## How To Use The Result In This Repository

After the PySide6-side documents are created:

1. Place only the handoff documents under `docs/05_handoff/pyside6-source/`
   or another clearly named handoff subdirectory.
2. Compare them with the current source-of-truth docs:
   - `docs/01_requirements/`
   - `docs/02_design/`
   - `docs/03_planning/01_remaining-tasks.md`
3. Convert confirmed gaps into a planning document under `docs/03_planning/`.
4. Do not add speculative backlog items to `AGENTS.md` or `CLAUDE.md`.
5. If Tauri migration resumes, preserve these project rules:
